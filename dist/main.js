...
window.addEventListener('map:changed', (e) => {
...
function updateAgentComments(context) {
    // Debug log
    const debugEl = document.getElementById('agent-debug');
    if (debugEl) {
        debugEl.textContent = 'Last context: ' + JSON.stringify(context);
    }
    ['wonderer', 'detective', 'storyteller'].forEach(agent => {
        const el = document.getElementById(agentCommentIds[agent]);
        if (el)
            el.textContent = agentComment(agent, context);
    });
}
// Initial context
let currentContext = { title: 'the current map view' };
updateAgentComments(currentContext);
// Listen for timeline updates (visible events)
window.addEventListener('timeline:updated', (e) => {
    const detail = e.detail;
...
window.addEventListener('event:focus', (e) => {
    const detail = e.detail;
