...
type ExtraSection = { label: string; text: string };

function normId(s: string): string {
  return s.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '').slice(0, 24) || 'sec';
}

function eventCard(ev: EventItem, extra?: string, sections: ExtraSection[] = []): string {
  ...
  if (base) tabs.push({ id: 'summary', label: 'Summary', content: `<div class="ins-summary">${base}</div>` });
  if (extra) tabs.push({ id: 'context', label: 'Context', content: `<div class="ins-extra">${extra}</div>` });
  sections.forEach(s => {
...
export function renderInsightsPanel(root: HTMLElement) {
  root.innerHTML = `<div class="ins-list"></div>`;
  const list = root.querySelector('.ins-list') as HTMLElement;
  if (!list) return;

  async function render(events: EventItem[]) {
    list.innerHTML = '';
    ...
    for (const ev of events) {
      ...
      if (q && q.length > 2) {
        extra = await fetchWikiSummary(q);
      }
      ...
      if (ev.category === 'nato') {
        ...
        if (ev.title) queries.push({ label: 'Background', q: norm(ev.title) });
      }
      // Limit to 2 extra fetches to avoid spamming
      for (let i = 0; i < Math.min(2, queries.length); i++) {
        const item = queries[i];
        const text = await fetchWikiSummary(item.q);
        if (text) sections.push({ label: item.label, text });
      }
  ...
  document.addEventListener('timeline:updated', (e: Event) => {
    const detail = (e as CustomEvent).detail;
    const events = (detail && Array.isArray(detail.events)) ? detail.events as EventItem[] : [];
    render(events);
  });
  ...
  root.addEventListener('click', (e: Event) => {
    ...
    if (!target || !target.classList || !target.classList.contains('ins-tab')) return;
    const tabId = target.getAttribute('data-tab');
    const card = target.closest('.ins-card') as HTMLElement | null;
    if (!card || !tabId) return;
    // Update tab active state
    card.querySelectorAll('.ins-tab').forEach(el => el.classList.remove('active'));
    target.classList.add('active');
    // Update panels
    card.querySelectorAll('.ins-panel').forEach((panel: Element) => {
      if ((panel as HTMLElement).getAttribute('data-panel') === tabId) {
